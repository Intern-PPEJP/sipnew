...
/**
 * Table class for cv_historipeserta
 */
class cv_historipeserta extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'cv_historipeserta';
		$this->TableName = 'cv_historipeserta';
		...
		// kdhistori
		$this->kdhistori = new DbField('cv_historipeserta', 'cv_historipeserta', 'x_kdhistori', 'kdhistori', 't_pp.kdhistori', 't_pp.kdhistori', 3, 25, -1, FALSE, 't_pp.kdhistori', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// kdpelat
		$this->kdpelat = new DbField('cv_historipeserta', 'cv_historipeserta', 'x_kdpelat', 'kdpelat', 't_pp.kdpelat', 't_pp.kdpelat', 200, 20, -1, FALSE, 't_pp.kdpelat', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// id
		$this->id = new DbField('cv_historipeserta', 'cv_historipeserta', 'x_id', 'id', 't_pp.id', 't_pp.id', 3, 11, -1, FALSE, 't_pp.id', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tahun
		$this->tahun = new DbField('cv_historipeserta', 'cv_historipeserta', 'x_tahun', 'tahun', 't_pp.tahun', 't_pp.tahun', 2, 6, -1, FALSE, 't_pp.tahun', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// kdinformasi
		$this->kdinformasi = new DbField('cv_historipeserta', 'cv_historipeserta', 'x_kdinformasi', 'kdinformasi', 't_pp.kdinformasi', 't_pp.kdinformasi', 3, 5, -1, FALSE, 't_pp.kdinformasi', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// harapan
		$this->harapan = new DbField('cv_historipeserta', 'cv_historipeserta', 'x_harapan', 'harapan', 't_pp.harapan', 't_pp.harapan', 201, 16777215, -1, FALSE, 't_pp.harapan', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->harapan->Sortable = TRUE; // Allow sort
		$this->fields['harapan'] = &$this->harapan;

		// sertifikat
		$this->sertifikat = new DbField('cv_historipeserta', 'cv_historipeserta', 'x_sertifikat', 'sertifikat', 't_pp.sertifikat', 't_pp.sertifikat', 200, 100, -1, FALSE, 't_pp.sertifikat', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getMasterFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "t_pelatihan") {
			if ($this->kdpelat->getSessionValue() != "")
				$masterFilter .= "`kdpelat`=" . QuotedValue($this->kdpelat->getSessionValue(), DATATYPE_STRING, "DB");
			else
				return "";
		}
		return $masterFilter;
	...
	public function getDetailFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "t_pelatihan") {
			if ($this->kdpelat->getSessionValue() != "")
				$detailFilter .= "t_pp.kdpelat=" . QuotedValue($this->kdpelat->getSessionValue(), DATATYPE_STRING, "DB");
			else
				return "";
		}
		return $detailFilter;
	...
	// Master filter
	public function sqlMasterFilter_t_pelatihan()
	{
		return "`kdpelat`='@kdpelat@'";
	}

	// Detail filter
	public function sqlDetailFilter_t_pelatihan()
	{
		return "t_pp.kdpelat='@kdpelat@'";
	}
	...
	{
		return ($this->SqlSelect != "") ? $this->SqlSelect : "SELECT t_pp.kdpelat, t_pp.id, t_pp.tahun, t_pp.kdhistori, t_pp.kdinformasi, t_pp.harapan, t_pp.sertifikat FROM " . $this->getSqlFrom();
	}
	...
	{
		return "t_pp.kdhistori = @kdhistori@";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "cv_historipesertalist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "cv_historipesertaview.php")
			return $Language->phrase("View");
		elseif ($pageName == "cv_historipesertaedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "cv_historipesertaadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("cv_historipesertaview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("cv_historipesertaview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "cv_historipesertaadd.php?" . $this->getUrlParm($parm);
		else
			$url = "cv_historipesertaadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("cv_historipesertaedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("cv_historipesertaadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("cv_historipesertadelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "t_pelatihan" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
			$url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_kdpelat=" . urlencode($this->kdpelat->CurrentValue);
		}
		return $url;
	}
	...
	function Row_Rendered() {
		...
		$cekpes = ExecuteScalar("SELECT COUNT(1) FROM t_pp WHERE kdpelat LIKE '". $GLOBALS['t_pelatihan']->kdpelat->CurrentValue."'");
		if($cekpes > 0){
			$thpel = ExecuteScalar("SELECT YEAR(tawal) FROM t_pelatihan WHERE kdpelat LIKE '". $GLOBALS['t_pelatihan']->kdpelat->CurrentValue."'");
			$th45 = $thpel-45;
			$pns = 0;
			$bumn = ExecuteScalar("SELECT COUNT(1) FROM t_pp a INNER JOIN t_peserta b ON a.id = b.id INNER JOIN t_perusahaan c ON b.idp = c.idp WHERE c.kdkategori = 1 AND a.kdpelat LIKE '". $GLOBALS['t_pelatihan']->kdpelat->CurrentValue."'");
			$kop = ExecuteScalar("SELECT COUNT(1) FROM t_pp a INNER JOIN t_peserta b ON a.id = b.id INNER JOIN t_perusahaan c ON b.idp = c.idp WHERE c.kdkategori = 7 AND a.kdpelat LIKE '". $GLOBALS['t_pelatihan']->kdpelat->CurrentValue."'");
			$independen = ExecuteScalar("SELECT COUNT(1) FROM t_pp a INNER JOIN t_peserta b ON a.id = b.id WHERE b.idp < 1 AND a.kdpelat LIKE '". $GLOBALS['t_pelatihan']->kdpelat->CurrentValue."'");
			$dosen = ExecuteScalar("SELECT COUNT(1) FROM t_pp a INNER JOIN t_peserta b ON a.id = b.id WHERE b.kdjabat = 8 AND a.kdpelat LIKE '". $GLOBALS['t_pelatihan']->kdpelat->CurrentValue."'");
			$mhs = ExecuteScalar("SELECT COUNT(1) FROM t_pp a INNER JOIN t_peserta b ON a.id = b.id WHERE b.kdjabat = 35 AND a.kdpelat LIKE '". $GLOBALS['t_pelatihan']->kdpelat->CurrentValue."'");
			$lk = ExecuteScalar("SELECT COUNT(b.kdsex) FROM t_pp a INNER JOIN t_peserta b ON a.id = b.id WHERE b.kdsex = 1 AND a.kdpelat LIKE '". $GLOBALS['t_pelatihan']->kdpelat->CurrentValue."'");
			$pr = ExecuteScalar("SELECT COUNT(b.kdsex) FROM t_pp a INNER JOIN t_peserta b ON a.id = b.id WHERE b.kdsex = 2 AND a.kdpelat LIKE '". $GLOBALS['t_pelatihan']->kdpelat->CurrentValue."'");
			$usia_k45 = ExecuteScalar("SELECT COUNT(1) FROM t_pp a INNER JOIN t_peserta b ON a.id = b.id WHERE YEAR(b.tlahir) > ".$th45." AND YEAR(tlahir) > 0 AND a.kdpelat LIKE '". $GLOBALS['t_pelatihan']->kdpelat->CurrentValue."'");
			$usia_b45 = ExecuteScalar("SELECT COUNT(1) FROM t_pp a INNER JOIN t_peserta b ON a.id = b.id WHERE YEAR(b.tlahir) <= ".$th45." AND  YEAR(tlahir) > 0 AND a.kdpelat LIKE '". $GLOBALS['t_pelatihan']->kdpelat->CurrentValue."'");;
			Execute("UPDATE `t_pelatihan` SET `real_peserta` = ".$cekpes.", independen = ".$independen.", bumn = ".$bumn.", koperasi = ".$kop.", `pt_dosen` = ".$dosen.", `pt_mhs` = ".$mhs.", `jk_l` = ".$lk.", `jk_p` = ".$pr.", usia_k45 = ".$usia_k45.", usia_b45 = ".$usia_b45." WHERE `kdpelat` LIKE '". $GLOBALS['t_pelatihan']->kdpelat->CurrentValue."'") or die("Execute Gagal - 1681");
		}
	}
