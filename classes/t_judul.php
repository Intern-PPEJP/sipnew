...
/**
 * Table class for t_judul
 */
class t_judul extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 't_judul';
		$this->TableName = 't_judul';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_judul`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// kdjudul
		$this->kdjudul = new DbField('t_judul', 't_judul', 'x_kdjudul', 'kdjudul', '`kdjudul`', '`kdjudul`', 200, 9, -1, FALSE, '`kdjudul`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kdjudul->IsPrimaryKey = TRUE; // Primary key field
		$this->kdjudul->IsForeignKey = TRUE; // Foreign key field
		$this->kdjudul->Nullable = FALSE; // NOT NULL field
		$this->kdjudul->Required = TRUE; // Required field
		$this->kdjudul->Sortable = TRUE; // Allow sort
		$this->fields['kdjudul'] = &$this->kdjudul;

		// kdbidang
		$this->kdbidang = new DbField('t_judul', 't_judul', 'x_kdbidang', 'kdbidang', '`kdbidang`', '`kdbidang`', 200, 10, -1, FALSE, '`kdbidang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// judul
		$this->judul = new DbField('t_judul', 't_judul', 'x_judul', 'judul', '`judul`', '`judul`', 201, 65535, -1, FALSE, '`judul`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->judul->Required = TRUE; // Required field
		$this->judul->Sortable = TRUE; // Allow sort
		$this->fields['judul'] = &$this->judul;

		// singkatan
		$this->singkatan = new DbField('t_judul', 't_judul', 'x_singkatan', 'singkatan', '`singkatan`', '`singkatan`', 200, 11, -1, FALSE, '`singkatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->singkatan->Required = TRUE; // Required field
		$this->singkatan->Sortable = TRUE; // Allow sort
		$this->fields['singkatan'] = &$this->singkatan;

		// created_at
		$this->created_at = new DbField('t_judul', 't_judul', 'x_created_at', 'created_at', '`created_at`', CastDateFieldForLike("`created_at`", 0, "DB"), 135, 19, 0, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_at->Sortable = TRUE; // Allow sort
		$this->created_at->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['created_at'] = &$this->created_at;

		// user_created_by
		$this->user_created_by = new DbField('t_judul', 't_judul', 'x_user_created_by', 'user_created_by', '`user_created_by`', '`user_created_by`', 200, 100, -1, FALSE, '`user_created_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->user_created_by->Sortable = TRUE; // Allow sort
		$this->fields['user_created_by'] = &$this->user_created_by;

		// updated_at
		$this->updated_at = new DbField('t_judul', 't_judul', 'x_updated_at', 'updated_at', '`updated_at`', CastDateFieldForLike("`updated_at`", 0, "DB"), 135, 19, 0, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->updated_at->Sortable = TRUE; // Allow sort
		$this->updated_at->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['updated_at'] = &$this->updated_at;

		// user_updated_by
		$this->user_updated_by = new DbField('t_judul', 't_judul', 'x_user_updated_by', 'user_updated_by', '`user_updated_by`', '`user_updated_by`', 200, 100, -1, FALSE, '`user_updated_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "t_juduldetail") {
			$detailUrl = $GLOBALS["t_juduldetail"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_kdjudul=" . urlencode($this->kdjudul->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "t_judullist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`t_judul`";
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('kdjudul', $rs))
				AddFilter($where, QuotedName('kdjudul', $this->Dbid) . '=' . QuotedValue($rs['kdjudul'], $this->kdjudul->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		// Cascade delete detail table 't_juduldetail'
		if (!isset($GLOBALS["t_juduldetail"]))
			$GLOBALS["t_juduldetail"] = new t_juduldetail();
		$rscascade = $GLOBALS["t_juduldetail"]->loadRs("`kdjudul` = " . QuotedValue($rs['kdjudul'], DATATYPE_STRING, "DB"));
		$dtlrows = ($rscascade) ? $rscascade->getRows() : [];

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["t_juduldetail"]->Row_Deleting($dtlrow);
			if (!$success)
				break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["t_juduldetail"]->delete($dtlrow); // Delete
				if (!$success)
					break;
			}
		}
		...
		if ($success) {
			foreach ($dtlrows as $dtlrow)
				$GLOBALS["t_juduldetail"]->Row_Deleted($dtlrow);
		}
		if ($success)
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('kdjudul', $row) ? $row['kdjudul'] : NULL;
		else
			$val = $this->kdjudul->OldValue !== NULL ? $this->kdjudul->OldValue : $this->kdjudul->CurrentValue;
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@kdjudul@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "t_juduledit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "t_juduladd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("t_judulview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t_judulview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "t_juduladd.php?" . $this->getUrlParm($parm);
		else
			$url = "t_juduladd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("t_juduledit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t_juduledit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("t_juduladd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t_juduladd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("t_juduldelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "kdjudul:" . JsonEncode($this->kdjudul->CurrentValue, "string");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->kdjudul->CurrentValue != NULL) {
			$url .= "kdjudul=" . urlencode($this->kdjudul->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("kdjudul") !== NULL)
				$arKeys[] = Param("kdjudul");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->kdjudul->CurrentValue = $key;
			else
				$this->kdjudul->OldValue = $key;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->singkatan);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->singkatan);
	...
	function Row_Inserting($rsold, &$rsnew) {
		...
		$rsnew["kdjudul"] = cs_getNextKode("jp",$rsnew["kdbidang"]);

		//$this->setWarningMessage("Kode Judul : ".$rsnew["kdjudul"]);
		return TRUE;
	}
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//echo "Row Inserted"	
		ob_end_clean();
		$this->setSuccessMessage("Penambahan judul pelatihan berhasil. <a href='t_judullist.php?cmd=reset'>Tampilkan semua data</a>");
		header("Location: t_judullist.php?cmd=search&t=t_judul&z_kdbidang=%3D&x_kdbidang=&z_judul=LIKE&x_judul=".$rsnew["judul"]);
		exit();
	}
	...
	function Row_Rendered() {
		...
		//	$this->kdjudul->CurrentValue = cs_getNextKode("jp");
		//	$this->kdjudul->EditValue = $this->kdjudul->CurrentValue;

			$this->kdjudul->ReadOnly = TRUE;
		}
		if(CurrentPageID()=="add" && $this->CurrentAction == "F"){
