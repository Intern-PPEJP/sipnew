...
/**
 * Table class for cv_historipelatihanpeserta
 */
class cv_historipelatihanpeserta extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'cv_historipelatihanpeserta';
		$this->TableName = 'cv_historipelatihanpeserta';
		...
		// kdhistori
		$this->kdhistori = new DbField('cv_historipelatihanpeserta', 'cv_historipelatihanpeserta', 'x_kdhistori', 'kdhistori', 't_pp.kdhistori', 't_pp.kdhistori', 3, 25, -1, FALSE, 't_pp.kdhistori', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// id
		$this->id = new DbField('cv_historipelatihanpeserta', 'cv_historipelatihanpeserta', 'x_id', 'id', 't_pp.id', 't_pp.id', 3, 11, -1, FALSE, 't_pp.id', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id->IsForeignKey = TRUE; // Foreign key field
		$this->id->Required = TRUE; // Required field
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->Lookup = new Lookup('id', 'vt_peserta', FALSE, 'id', ["nama","namap","",""], [], [], [], [], [], [], '', '');
		$this->id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// kdpelat
		$this->kdpelat = new DbField('cv_historipelatihanpeserta', 'cv_historipelatihanpeserta', 'x_kdpelat', 'kdpelat', 't_pp.kdpelat', 't_pp.kdpelat', 200, 20, -1, FALSE, 't_pp.kdpelat', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kdpelat->Required = TRUE; // Required field
		$this->kdpelat->Sortable = TRUE; // Allow sort
		$this->kdpelat->Lookup = new Lookup('kdpelat', 'vt_pelatihan', FALSE, 'kdpelat', ["judul","kota","tawal",""], [], [], [], [], [], [], '', '');
		$this->fields['kdpelat'] = &$this->kdpelat;

		// tahun
		$this->tahun = new DbField('cv_historipelatihanpeserta', 'cv_historipelatihanpeserta', 'x_tahun', 'tahun', 't_pp.tahun', 't_pp.tahun', 2, 6, -1, FALSE, 't_pp.tahun', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// kdinformasi
		$this->kdinformasi = new DbField('cv_historipelatihanpeserta', 'cv_historipelatihanpeserta', 'x_kdinformasi', 'kdinformasi', 't_pp.kdinformasi', 't_pp.kdinformasi', 3, 5, -1, FALSE, 't_pp.kdinformasi', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// harapan
		$this->harapan = new DbField('cv_historipelatihanpeserta', 'cv_historipelatihanpeserta', 'x_harapan', 'harapan', 't_pp.harapan', 't_pp.harapan', 201, 16777215, -1, FALSE, 't_pp.harapan', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->harapan->Sortable = TRUE; // Allow sort
		$this->fields['harapan'] = &$this->harapan;

		// sertifikat
		$this->sertifikat = new DbField('cv_historipelatihanpeserta', 'cv_historipelatihanpeserta', 'x_sertifikat', 'sertifikat', 't_pp.sertifikat', 't_pp.sertifikat', 200, 100, -1, FALSE, 't_pp.sertifikat', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "t_peserta") {
			if ($this->id->getSessionValue() != "")
				$detailFilter .= "t_pp.id=" . QuotedValue($this->id->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $detailFilter;
	...
	// Master filter
	public function sqlMasterFilter_t_peserta()
	{
		return "`id`=@id@";
	}

	// Detail filter
	public function sqlDetailFilter_t_peserta()
	{
		return "t_pp.id=@id@";
	}
	...
	{
		return ($this->SqlSelect != "") ? $this->SqlSelect : "SELECT t_pp.id, t_pp.kdpelat, t_pp.tahun, t_pp.kdhistori, t_pp.kdinformasi, t_pp.harapan, t_pp.sertifikat FROM " . $this->getSqlFrom();
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "cv_historipelatihanpesertalist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "cv_historipelatihanpesertaedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "cv_historipelatihanpesertaadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("cv_historipelatihanpesertaview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("cv_historipelatihanpesertaview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "cv_historipelatihanpesertaadd.php?" . $this->getUrlParm($parm);
		else
			$url = "cv_historipelatihanpesertaadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("cv_historipelatihanpesertaedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("cv_historipelatihanpesertaadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("cv_historipelatihanpesertadelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "t_peserta" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
	...
	{
		$table = 'cv_historipelatihanpeserta';
		$usr = CurrentUserName();
	...
	function Row_Rendered() {
		...
		$this->kdpelat->ViewValue = '<a href="t_pelatihanview.php?showdetail=&kdpelat='.$this->kdpelat->CurrentValue.'">'.$this->kdpelat->ViewValue.'</a>';
	}
