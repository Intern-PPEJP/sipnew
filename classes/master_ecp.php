...
/**
 * Table class for master_ecp
 */
class master_ecp extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'master_ecp';
		$this->TableName = 'master_ecp';
		...
		// id
		$this->id = new DbField('master_ecp', 'master_ecp', 'x_id', 'id', 't_pcp.id', 't_pcp.id', 3, 10, -1, FALSE, 't_pcp.id', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// rkid
		$this->rkid = new DbField('master_ecp', 'master_ecp', 'x_rkid', 'rkid', 't_pcp.rkid', 't_pcp.rkid', 3, 10, -1, FALSE, 't_pcp.rkid', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// wilayah_ecp
		$this->wilayah_ecp = new DbField('master_ecp', 'master_ecp', 'x_wilayah_ecp', 'wilayah_ecp', 't_pcp.wilayah_ecp', 't_pcp.wilayah_ecp', 200, 100, -1, FALSE, 't_pcp.wilayah_ecp', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// tahun_ecp
		$this->tahun_ecp = new DbField('master_ecp', 'master_ecp', 'x_tahun_ecp', 'tahun_ecp', 't_pcp.tahun_ecp', 't_pcp.tahun_ecp', 2, 4, -1, FALSE, 't_pcp.tahun_ecp', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// nama_peserta
		$this->nama_peserta = new DbField('master_ecp', 'master_ecp', 'x_nama_peserta', 'nama_peserta', 't_pcp.nama_peserta', 't_pcp.nama_peserta', 200, 100, -1, FALSE, 't_pcp.nama_peserta', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// namap
		$this->namap = new DbField('master_ecp', 'master_ecp', 'x_namap', 'namap', 't_pcp.namap', 't_pcp.namap', 200, 150, -1, FALSE, 't_pcp.namap', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// produk
		$this->produk = new DbField('master_ecp', 'master_ecp', 'x_produk', 'produk', 't_pcp.produk', 't_pcp.produk', 200, 255, -1, FALSE, 't_pcp.produk', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->produk->Sortable = TRUE; // Allow sort
		$this->produk->Lookup = new Lookup('produk', 't_tahun', FALSE, 'tahun', ["tahun","","",""], [], [], [], [], [], [], '`tahun` ASC', '');
	...
	{
		return ($this->SqlSelect != "") ? $this->SqlSelect : "SELECT t_pcp.id, t_pcp.rkid, t_pcp.wilayah_ecp, t_pcp.tahun_ecp, t_pcp.nama_peserta, t_pcp.namap, t_pcp.produk FROM " . $this->getSqlFrom();
	}
	...
	{
		global $Language;
		if ($pageName == "master_ecpview.php")
			return $Language->phrase("View");
		elseif ($pageName == "master_ecpedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "master_ecpadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("master_ecpview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("master_ecpview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "master_ecpadd.php?" . $this->getUrlParm($parm);
		else
			$url = "master_ecpadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("master_ecpedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("master_ecpadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("master_ecpdelete.php", $this->getUrlParm());
	}
	...
	public function renderListRow()
	{
		...
		if ($curVal != "") {
			...
			if ($this->namap->ViewValue === NULL) { // Lookup from database
				$filterWrk = "`namap`" . SearchString("=", $curVal, DATATYPE_STRING, "");
		...
		$curVal = strval($this->produk->CurrentValue);
		if ($curVal != "") {
			$this->produk->ViewValue = $this->produk->lookupCacheOption($curVal);
			if ($this->produk->ViewValue === NULL) { // Lookup from database
				$filterWrk = "`tahun`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
				$lookupFilter = function() {
					return "`tahun` > 2010";
				};
				$lookupFilter = $lookupFilter->bindTo($this);
				$sqlWrk = $this->produk->Lookup->getSql(FALSE, $filterWrk, $lookupFilter, $this);
				$rswrk = Conn()->execute($sqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$arwrk = [];
					$arwrk[1] = $rswrk->fields('df');
					$this->produk->ViewValue = $this->produk->displayValue($arwrk);
					$rswrk->Close();
				} else {
					$this->produk->ViewValue = $this->produk->CurrentValue;
				}
