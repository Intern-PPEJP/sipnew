...
class audittrail extends DbTable
{
	...
	public function __construct()
	{
		...
		// script
		$this->script = new DbField('audittrail', 'audittrail', 'x_script', 'script', '`script`', '`script`', 200, 255, -1, FALSE, '`script`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->script->Sortable = TRUE; // Allow sort
		$this->fields['script'] = &$this->script;

		// user
		$this->user = new DbField('audittrail', 'audittrail', 'x_user', 'user', '`user`', '`user`', 200, 255, -1, FALSE, '`user`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->user->Sortable = TRUE; // Allow sort
		$this->fields['user'] = &$this->user;

		// action
		$this->_action = new DbField('audittrail', 'audittrail', 'x__action', 'action', '`action`', '`action`', 200, 255, -1, FALSE, '`action`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->_action->Sortable = TRUE; // Allow sort
		$this->fields['action'] = &$this->_action;

		// table
		$this->_table = new DbField('audittrail', 'audittrail', 'x__table', 'table', '`table`', '`table`', 200, 255, -1, FALSE, '`table`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->_table->Sortable = TRUE; // Allow sort
		$this->fields['table'] = &$this->_table;

		// field
		$this->field = new DbField('audittrail', 'audittrail', 'x_field', 'field', '`field`', '`field`', 200, 255, -1, FALSE, '`field`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
